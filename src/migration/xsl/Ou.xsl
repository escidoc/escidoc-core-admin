...
xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" exclude-result-prefixes="fedoraxsi xsl rdf audit">
	...
	<xsl:template name="ouTemplate">
				...
				<xsl:for-each select="foxml:datastream">
					<xsl:choose>
						...
						<xsl:when test="foxml:datastreamVersion/foxml:xmlContent/organizational-unit:organization-details">
							<xsl:element name="foxml:datastream" namespace="info:fedora/fedora-system:def/foxml#">
								...
								<xsl:for-each select="foxml:datastreamVersion">
									...
									<xsl:element name="foxml:datastreamVersion" namespace="info:fedora/fedora-system:def/foxml#">
										<xsl:attribute name="ALT_IDS" select="'metadata unknown unknown'"/>
										<xsl:attribute name="CREATED" select="@CREATED"/>
										<xsl:attribute name="ID" select="concat('escidoc.',$counter)"/>
										...
										<xsl:element name="foxml:xmlContent" namespace="info:fedora/fedora-system:def/foxml#">
											<xsl:element name="ou:ou" namespace="http://www.escidoc.de/metadata/organizational-unit">
												<xsl:for-each select="foxml:xmlContent/organizational-unit:organization-details/*">
													<xsl:variable name="name" select="name()"/>
													<xsl:choose>
														<xsl:when test="$name = 'organizational-unit:name'">
															<xsl:element name="dc:title" namespace="http://purl.org/dc/elements/1.1/">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:when>
														<xsl:when test="$name = 'organizational-unit:abbreviation'">
															<xsl:element name="dcterms:alternative" namespace="http://purl.org/dc/terms/">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:when>
														<xsl:when test="$name = 'organizational-unit:description'">
															<xsl:element name="dc:description" namespace="http://purl.org/dc/elements/1.1/">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:when>
														<xsl:when test="$name = 'organizational-unit:external-id'">
															<xsl:element name="dc:identifier" namespace="http://purl.org/dc/elements/1.1/">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:when>
														<xsl:when test="$name = 'organizational-unit:organization-type'">
															<xsl:element name="ou:organization-type" namespace="http://www.escidoc.de/metadata/organizational-unit">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:when>		
														<xsl:when test="$name = 'organizational-unit:geo-coordinate'">
														<xsl:variable name="longitude" select="organizational-unit:location-longitude"/>
														<xsl:variable name="latitude" select="organizational-unit:location-latitude"/>	
																<xsl:element name="georss:point" namespace="http://www.georss.org/">
																	<xsl:value-of select="concat($latitude,' ', $longitude)"/>
																</xsl:element>
														</xsl:when>
														<xsl:otherwise>
															<xsl:element name="ou:{local-name()}" namespace="http://www.escidoc.de/metadata/organizational-unit">
																<xsl:value-of select="text()"/>
															</xsl:element>
														</xsl:otherwise>
													</xsl:choose>
												</xsl:for-each>
												<xsl:element name="ou:start-date" namespace="http://www.escidoc.de/metadata/organizational-unit"/>
												<xsl:element name="ou:end-date" namespace="http://www.escidoc.de/metadata/organizational-unit"/>
											...
											<xsl:element name="rdf:RDF" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
												<xsl:element name="rdf:Description" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
													...
													<xsl:for-each select="foxml:xmlContent/rdf:RDF/rdf:Description/*">
														<xsl:variable name="name" select="local-name()"/>
														<xsl:if test="$name='hasParent'">
															<xsl:element name="srel:parent" namespace="http://escidoc.de/core/01/structural-relations/">
														...
														<xsl:if test="$name='objectType'">
															<xsl:element name="rdf:type" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
																<xsl:attribute name="rdf:resource" namespace="http://www.w3.org/1999/02/22-rdf-syntax-ns#">http://escidoc.de/core/01/resources/OrganizationalUnit</xsl:attribute>
															</xsl:element>
														</xsl:if>
													</xsl:for-each>
												</xsl:element>
