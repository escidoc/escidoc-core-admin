...
CREATE TABLE aa.user_account_ous (
  ...
  ou_id VARCHAR(255) NOT NULL,
   primary key (list_index, user_account_id),
   CONSTRAINT FK_UA_ID FOREIGN KEY (user_account_id) REFERENCES aa.user_account (id)
);
...
CREATE TABLE aa.role_grant (
  ...
  primary key (id),
  CONSTRAINT FK_GRANT FOREIGN KEY (user_id) REFERENCES aa.user_account (id),
